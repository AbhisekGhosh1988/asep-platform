package com.abhisek.asep.generator.backend.java.domainevent;
...
@Slf4j
@Component
public class DefaultJavaDomainEventGenerator extends AbstractJavaArtifactGenerator implements JavaDomainEventGenerator {

    public DefaultJavaDomainEventGenerator(TemplateRepository templateRepository, TemplateEngine templateEngine, SourceWriter sourceWriter) {
    ...
    public void generate(GenerationTask step, CompilerResult compilerResult, GenerationContext generationContext) {
        ...
        if (!(step.getNode() instanceof DomainEventIR event)) {

            return;

        }

        Template template = loadTemplate("domain-event.java.tpl");

        TemplateContext context = new TemplateContext().put("package", generationContext.getBasePackage() + ".domain.event").put("className", event.getName()).put("fields", buildFields(event));

        String source = render(template, context);

        writeSource(resolveJavaFile(generationContext, generationContext.getBasePackage() + ".domain.event", event.getName()), source);

        log.info("Generated Domain Event {}", event.getName());

    }

    private String buildFields(DomainEventIR event) {

        StringBuilder builder = new StringBuilder();

        for (AttributeIR attribute : event.getAttributes()) {
