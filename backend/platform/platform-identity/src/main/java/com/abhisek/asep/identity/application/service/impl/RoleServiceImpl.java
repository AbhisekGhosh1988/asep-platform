...
public class RoleServiceImpl implements RoleService {
    ...
    public RoleResponse create(CreateRoleRequest request) {
        ...
        Role role = Role.builder().roleType(request.getRoleType()).build();

        role = roleRepository.save(role);

        return RoleResponse.builder().id(role.getId()).roleType(role.getRoleType()).build();
    }

    @Override
    public List<RoleResponse> findAll() {

        return roleRepository.findAll().stream().map(role -> RoleResponse.builder().id(role.getId()).roleType(role.getRoleType()).build()).toList();
    }

    @Override
    public RoleResponse findById(String id) {

        Role role = roleRepository.findById(id).orElseThrow(() -> new RoleNotFoundException("Role not found: " + id));
